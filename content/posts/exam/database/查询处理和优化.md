...
- 查询处理
- 查询优化
	- 代数：关系代数，如尽早选择，尽早投影，笛卡尔积和选择合并（等值、自然连接）
	- 物理：存取路径，底层算法，如选择索引或全表扫描，用什么连接算法

大题：画语法树，优化后的语法树

查询处理：DBMS执行语句的过程
- 查询分析：对语句进行词法，语法分析
- 查询检查：初步、静态检查（属性名是否存在，权限，完整性约束）；转化为查询树（语法分析树）
- 查询优化：选择高效的执行策略；代数，物理优化
- 查询执行：优化器（生成查询执行计划） > 代码生成器 > 执行代码

查询优化的重要性
- 让用户不用考虑执行路径
- 优化器可以在数据字典中获取很多统计信息（如索引）

选择操作select
- table scan全表：从磁盘获取数据到内存，对于内存中的每个元组，如果满足条件，则输出；适合数据选择率高的场景
- index scan索引：聚簇索引和非聚簇（回表）；通过索引找到数据；适用于选择率低的场景

连接操作join
- 嵌套循环nested loop join：遍历外层，遍历内层，如果满足连接条件，则返回
- 排序-合并merge join
- 索引index join：通过索引找内表，连接
- hash join：等值连接；build, probe
